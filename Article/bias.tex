...
\begin{document}
\title{Bias Bias In Proton Radius Extractions From Electron Scattering Data} 
...
We will show, that for a given set of data, that the parsimonious model can in fact be the better model to use.
...
new generation of published atomic result is in fact in statistical agreement with the muonic result~\cite{}
though preliminary results from another group agrees with the classic atomic results~\cite{}.
...
extremely complex models with more than fifty free parameters~\cite{}.   As nicely illistrated in the
work of Krauth~{\it{\et al.}}~\cite{Krauth:2017ijq}, the new parsimonious fits tend to agree with the 
muonic results while the more complex fits are generally in agreement with the CODATA value.
...
One of the criticisms of the parsimonious extractions of the proton radius is the prescence of bias~\cite{}.
with an implication that bias needs to be avoided in order to successfully extract the true radius from the data.
In fact, the rejection of statistical models based solely on bias was explicately stated in the 
classic Monte Carlo study of Borkowski {\it{et al.}}~\cite{Borkowski:1975}. 

We will show in this work that when building a statistical model to extract the proton radius, one needs 
to in fact consider both bias and variance.   And thus, contrary to intuition, models with some bias 
can in fact have better predictive power then models without bias.   In fact, the idea of balancing bias and 
varaince forms the heart of machine learning as well as many statistical modeing techniques~\cite{}.
