...
tends to agree with the CODATA value.   One detail that is affecting all the results is how the normalization
...
even the low order terms of $G_M$ do not alternate in sign as one would expect~\cite{Bernauer:2010zga}.
Therefore, we claim that these terms are simply parameters of curve fitting and not 
moments of an underlying true function.   This observation is in line with nuclear theory, which has shown 
that it take hundreds of moments to fully describe the range of the Mainz data.
HERE HERE HERE
The selection of the appropriate regression model for a particular task requires a rigorous mathematical basis
following standard statistical criteria. Idealy, the procedure should be defined prior to seeing the experimental data.
For example, the PRad collaboration has come up with their model selection techniques
...
presented herein provide a rigorous method of selecting the most appropriate model to describe a given set of data.  
...
While the Frequentist
seeks the best model to describe the data, a Bayesian would likely look at the collection of results as  
a subset of the possible ways the data can be normalized and the radius extracted (e.g. Gaussian process 
...
These ideas are not limited to the physical sciences, but also extend
