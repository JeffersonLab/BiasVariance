...
In Fig.~\ref{residual}, the reported data is shown along with the best fit curve from Table~\ref{datatable} and while a residual
is perhaps the most common statistical analysis graph, it is not found.   In fact, some papers have no data qualtiy plots at all 
(e.g.~\cite{Rosenfelder:1999cd}) and residuals cannot be found in many of the most cited works~\cite{}.

\begin{figure}[htb]
\includegraphics[width=\columnwidth]{Figure/NewVsOld.pdf}
\caption{Data and the best fit curve.}
\label{residual}
\end{figure}

This plot allows one to see if the data plausibly come from a normal disutribition.
\begin{figure}[htb]
