...
\caption{Illustation the effect of the effect renormalizing of data.  The
...
See the supplimentary material for an animation of all the individual fits and how overfitting causes
the normalization and radius to shift dramatically.
}  
...
\begin{align}
\chi^2         & = \sum_{n=1}^{N}((data_i - model) / \sigma_i)^2, \\
reduced~\chi^2 & = \chi^2/ (N - N_{var}),  \\
AIC            & = N ln(\chi^2/N) + 2 N_{var}, \\
BIC            & = N ln(\chi^2/N) + ln(N) N_{var},
\end{align}
where $N$ is the number of data points, $data_i$ and $\sigma_i$ are measured values and estimated uncertainties,
and$N_{var}$ is the number of model parameters.
...
We note that the low order rational function (n=m=1) can be easily extended to give the expected asymtotic behavior
as $Q^2 -> \inf$ and that the more complex rational function (n=1,m=1,2,3) has been used in global fits~\cite{
Kelly:2004hm,
Puckett:2017flj,   
Gutsche:2017lyu},
...
Oddly enough the standard dipole magnetic form factor quadratic fit has the lowest AIC and BIC and gives a result consistant
with the muonic lamb shift; though the rational function is nearly as good and sitting nearly exactly between the CODATA
and muonic Lamb shiftvalues.    
