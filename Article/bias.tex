...
%   = \left( -6  \left. \frac{\partial G_E(Q^2)}{\partial Q^2}
...
Randomly generate pseudo sets of form factor data 
...
To mimic real data, the data points were randomly smeared with a normal distrubution
with a sigma of 0.5\%.  
...
Thus, the choice of the parsimonious modeler to use the low $Q^2$ data would likely be 
...
for example one may be interested in not only the proton's radius but also in the higher 
moments in $r$~\cite{Alarcon:2017hg}.
Thus, the tension in the extractions of the proton radius from electron scattering data is really 
about the fact that modelers using the low $Q^2$ are getting a systematically different result than
...
fixed, a spacing of 0.05~fm$^{-2}$ (7 points) and 0.005~fm$^{-2}$ (31 points) was used
...
\caption{Illistation the effect of the effect renormalizing of data using
one set of pseudo data, with absolute 0.3\% random point-to-point uncertainty and 0.005 fm$^{-2}$ spacing,
we show the effect of applying the prior that $G_E(0)=1$.   While the prior is true, using an inappropriate model
...
if the function cannot reasonably be expected to extrapolate beyond the data.}
\label{linearVSquadratic}
\end{figure}

To be clear, using an innapriate model can lead to erroneous conclusions.
...
nested models or the more general Akaike information criterion (AIC)~\cite{Akaike:1974} or Bayesian information criterion (BIC)~\cite{Schwarz:1978} 
(BIC) to guide our selection of the most appropriate model to describe a given set of data.
...
cross sections is nearly impossible, the absolute normalization of the cross section has typically been floated.  
...
\end{equation}
The known $\sigma_{\text{Mott}}$ term and determined normalization can be divided out to report a reduced cross section
\begin{equation}
...
To illustrate the current tension between fit done with different models, in Fig.~\ref{RealData} shows 
data points shows one full subset of the Mainz data~\footnote{Mainz Spectrometer B with a beam energy of 315 MeV with Mo and Tsi radiative correction downloaded
from supplimental material~\cite{Bernauer:2013tpr}.} 
that covers a range similar to those range studied herein.  By using a single set, only one floating normalization
parameter is reguired.
...
Additionally, we did fits with the following two common functions:
\begin{align}
f_{\mathrm{cubic}}(Q^2)   & = 1 + a_1 Q^2 + a_2 Q^4 + a_3 Q^6  \\
f_{\mathrm{rational}}(Q^2) & = (1+n_1 Q^2)/(1+m_1 Q^2).
\end{align}
where the radius for the rational function is given by $\sqrt(-6 (n_1 - m_1))$ and for the polynomials $\sqrt(-6 a_1)$.
Fits were done first with a standard dipole magnetic form factor
...
For the uncertainties, we used a statistical bootstrap.
It is important to note that the use of statistical criteria for model selection helps avoids confirmation bias,
though one could still be using inappropriate function for the problem at hand. 

\begin{table}
\caption{Using three different magnetic form factor parameterizations, we extract the normalization
and radius using a linear, quadaritic and cubic approximation for the $G_E$ function and Eq.~\ref{Eq:CrossSection}.
To avoid floating multiple normalizations, a single set of Spectrometer~B 315~MeV Mainz data nice shown in Fig.~\ref{RealData}.
The parameter uncertainies were obtained using $10^5$ statistical bootstraps of the data.}
...
linear    & 1.187        & 19.82        & {\bf{25.11}}    & 1.001(1)    & 0.853(2)   \\
quadratic & {\bf{1.173}} & {\bf{19.52}} & 27.45           & 0.998(2)    & 0.837(10)  \\
cubic     & 1.174        & 20.59        & 31.18           & 0.998(6)    & 0.788(57)  \\    
rational  & 1.173        & 19.54        & 27.47           & 0.998(2)    & 0.837(10)    \\    \hline \hline
\multicolumn{6}{l}{Bernauer Magnetic Form Factor}      \\  \hline
$G_E$ Fit       & reduced  & AIC   & BIC   & Norm & Radius    \\  
Function  & $\chi^2$ &       &       &               & [fm]      \\  \hline
linear    & 1.601    & 50.96 & 56.24 & 0.988(1)       & 0.786(2)     \\
quadratic & 1.183    & {\bf{20.43}} & {\bf{28.37}} & 1.000(4)        & 0.854(10)    \\ 
cubic     & {\bf{1.173}}    & 20.55 & 31.13        & 0.992(6)        & 0.785(56)    \\ 
rational  & 1.189        & 20.93        & 28.86    & 1.001(2)        & 0.861(13)    \\    \hline \hline
\multicolumn{6}{l}{Ye Magnetic Form Factor}      \\  \hline
$G_E$ Fit  & reduced  & AIC    & BIC   & Norm       & Radius    \\  
Function  & $\chi^2$  &        &       &            & [fm]      \\  \hline
linear    & 1.639     & 53.35  & 58.64 & 0.987(1)   & 0.781(2)     \\
quadratic & 1.182     & {\bf{20.33}}  & {\bf{28.26}} & 1.000(2)      & 0.852(10)     \\
cubic     & {\bf{1.173}}  & 20.55  & 31.12 & 0.992(6)  & 0.782(55)    \\ 
rational  & 1.188        & 20.86        & 28.80        & 1.001(2)    & 0.861(14)    \\    \hline \hline
\end{tabular}
\label{datatable}
\end{table}
...
radius extracted from electron scattered remained basically unchanged since the 1976 work~\cite{Borkowski:1975} while the 
functions used to make the extractions and obtained that same radius have became increasingly complex and convoluted.
